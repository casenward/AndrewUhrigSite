...
function previousPhoto() {
    currentPhotoIndex = (currentPhotoIndex - 1 + home_carousel.length) % home_carousel.length;
    updatePhoto();
    startAutoRotate(); // Restart the interval
}

function nextPhoto() {
    currentPhotoIndex = (currentPhotoIndex + 1) % home_carousel.length;
    updatePhoto();
    startAutoRotate(); // Restart the interval
}
...
function updatePhoto() {
    ...
    currentPhoto.src = home_carousel[currentPhotoIndex];

    // Update previous photo
    const previousIndex = (currentPhotoIndex - 1 + home_carousel.length) % home_carousel.length;
    previousPhoto.src = home_carousel[previousIndex];

    // Update next photo
    const nextIndex = (currentPhotoIndex + 1) % home_carousel.length;
    nextPhoto.src = home_carousel[nextIndex];
}
